...
class Settings():
    ...
    async def get_thumbnail(self) -> str | None:
       return CUSTOM_THUMBNAILS.get(self.chat_id)
    
    async def set_thumbnail(self, path):
       CUSTOM_THUMBNAILS[self.chat_id] = path
       logging.info(f"User {self.chat_id} set custom thumbnail: {path}")
    
    async def remove_thumbnail(self):
      CUSTOM_THUMBNAILS.pop(self.chat_id, None)
      logging.info(f"User {self.chat_id} removed custom thumbnail.")

    async def get_word_replacements(self) -> typing.Dict[str, str] | None:
       return CUSTOM_WORD_REPLACEMENTS.get(self.chat_id)

    async def set_word_replacements(self, replacements):
       CUSTOM_WORD_REPLACEMENTS[self.chat_id] = replacements
       logging.info(f"User {self.chat_id} set custom word replacements: {replacements}")
    
    async def delete_word_replacements(self, words):
       replacements = CUSTOM_WORD_REPLACEMENTS.get(self.chat_id, {})
       for word in words:
          replacements.pop(word, None)
       CUSTOM_WORD_REPLACEMENTS[self.chat_id] = replacements
       logging.info(f"User {self.chat_id} deleted custom word replacements: {words}")

    async def get_custom_caption(self) -> str | None:
        return CUSTOM_CAPTIONS.get(self.chat_id)

    async def set_custom_caption(self, caption):
        CUSTOM_CAPTIONS[self.chat_id] = caption
        logging.info(f"User {self.chat_id} set custom caption: {caption}")

    async def get_custom_channel(self) -> str | None:
       return CUSTOM_CHANNEL.get(self.chat_id)

    async def set_custom_channel(self, channel_id):
      CUSTOM_CHANNEL[self.chat_id] = channel_id
      logging.info(f"User {self.chat_id} set custom channel ID: {channel_id}")

    async def set_forward_status(self, status: bool):
       ENABLE_FORWARD[self.chat_id] = status
       logging.info(f"User {self.chat_id} set forwarding to {status}")
    
    async def get_forward_status(self) -> bool | None:
        return ENABLE_FORWARD.get(self.chat_id, False)
    
    async def set_error_status(self, status: bool):
       ENABLE_ERRORS[self.chat_id] = status
       logging.info(f"User {self.chat_id} set errors to {status}")

    async def get_error_status(self) -> bool | None:
        return ENABLE_ERRORS.get(self.chat_id, False)

    async def set_auto_reaction(self, emoji):
        AUTO_REACT_EMOJIS[self.chat_id] = emoji
        logging.info(f"User {self.chat_id} set auto reaction emoji : {emoji}")

    async def get_auto_reaction(self) -> str | None:
        return AUTO_REACT_EMOJIS.get(self.chat_id)
...
async def settings_command(client: Client, message: Message):
    ...
    buttons = [[
        ...
        InlineKeyboardButton("Set Auto Reaction", callback_data="set_reaction")
    ]]
    reply_markup = InlineKeyboardMarkup(buttons)
...
async def remove_thumb_query(client:Client, callback_query: pyrogram.types.CallbackQuery):
    settings = Settings(callback_query.from_user.id)
    await settings.remove_thumbnail()
...
async def enable_forward_query(client:Client, callback_query: pyrogram.types.CallbackQuery):
    settings = Settings(callback_query.from_user.id)
    current_status = await settings.get_forward_status()
    await settings.set_forward_status(not current_status)
...
async def enable_errors_query(client:Client, callback_query: pyrogram.types.CallbackQuery):
    settings = Settings(callback_query.from_user.id)
    current_status = await settings.get_error_status()
    await settings.set_error_status(not current_status)
    status = "enabled" if not current_status else "disabled"
    await callback_query.message.reply_text(f"Errors {status}.")

@Client.on_callback_query(filters.regex("set_reaction"))
async def set_reaction_query(client:Client, callback_query: pyrogram.types.CallbackQuery):
    await callback_query.message.reply_text("Send the emoji you want to automatically react to commands")
...
async def handle_custom_thumb(client: Client, message: Message):
    ...
    if message.reply_to_message and message.reply_to_message.text == "Send your custom thumbnail as a photo.":
        settings = Settings(message.from_user.id)
        path = await client.download_media(message)
        await settings.set_thumbnail(path)
     ...
     if message.reply_to_message:
         if message.reply_to_message.text == "Send the word replacements in JSON format (e.g., `{\"old_word\": \"new_word\", \"other_word\":\"new_word2\"}`)" :
            try:
               ...
               await settings.set_word_replacements(word_replacements)
             ...
             try:
                ...
                if not isinstance(words_to_delete, list):
                    ...
                    return
                await settings.delete_word_replacements(words_to_delete)
                ...
                settings = Settings(message.from_user.id)
                await settings.set_custom_caption(message.text)
            ...
            try:
              ...
              await settings.set_custom_channel(channel_id)
         ...
         elif message.reply_to_message.text == "Send the emoji you want to automatically react to commands":
            settings = Settings(message.from_user.id)
            await settings.set_auto_reaction(message.text)
            await message.reply_text("Auto reaction emoji set")
...
async def handle_message(client: Client, message: Message):
    ...
    if await settings.get_auto_reaction():
        try:
            await client.send_reaction(message.chat.id, message.id, await settings.get_auto_reaction())
        except Exception as e:
    ...
    with BATCH_PROCESSING_LOCKS[user_id]:
      ...
      for msgid in range(fromID, toID + 1):
            ...
            try:
               ...
               if await settings.get_error_status():
...
async def process_message(client: Client, message: Message, chatid, msgid):
    ...
    if msg_type == "Text":
      try:
          if await settings.get_forward_status() and await settings.get_custom_channel():
              custom_caption = apply_custom_caption(msg.text, user_id)
              await client.send_message(await settings.get_custom_channel(), custom_caption, entities = msg.entities, parse_mode=enums.ParseMode.HTML)
          else:
              custom_caption = apply_custom_caption(msg.text, user_id)
    ...
    try:
      if await settings.get_forward_status() and await settings.get_custom_channel():
          send_chat = await settings.get_custom_channel()
      else:
    ...
    thumbnail_path = await settings.get_thumbnail()
