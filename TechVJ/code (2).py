...
# handle private
async def handle_private(client: Client, acc, message: Message, chatid, msgid):
    ...
    if "Text" == msg_type:
        try:
            if await settings.get_forward_status() and await settings.get_custom_channel():
              ...
              custom_caption = apply_custom_caption(msg.text, user_id)
              await client.send_message(chat, custom_caption, entities=msg.entities, reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML)
        except Exception as e:
           if await settings.get_error_status():
    ...
    try:
      ...
      await handle_media_forward(client, message, acc, smsg, chat, msg, msg_type, progress)
    except Exception as e:
        log_error(f"Failed to process message in handle_private {msgid} from {chatid} due to error {e}")
        if await settings.get_error_status():
            await client.send_message(message.chat.id, f"Error: {e}", reply_to_message_id=message.id)

async def handle_media_forward(client, message, acc, smsg, chat, msg, msg_type, progress):
    user_id = message.from_user.id
    settings = Settings(user_id)
    file = f"{message.id}.file"
    ...
    dosta = asyncio.create_task(downstatus(client, status_file_down, smsg))
    ...
    try:
        ...
        try:
            ...
            log_error(f"Error downloading media: {e}")
            if await settings.get_error_status():
                await client.send_message(message.chat.id, f"Error: {e}", reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML)
            await dosta # Make sure this is awaited. This will raise the exception.
            return
        
        if os.path.exists(status_file_down):
          os.remove(status_file_down)

        upsta = asyncio.create_task(upstatus(client, status_file_up, smsg))
