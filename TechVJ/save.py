...
async def handle_message(client: Client, acc: Client, message: Message, msg: Message, user_data: dict):
    ...
    if CUSTOM_CHANNEL_ID:
        try:
            if msg_type == "Document":
                await acc.send_message(CUSTOM_CHANNEL_ID, msg.document, entities=msg.entities)
            else:
                await acc.copy_message(CUSTOM_CHANNEL_ID, msg.chat.id, msg.id)
        except Exception as e:
            print(f"Failed to forward message to custom channel: {e}")
    ...
    if CUSTOM_CHANNEL_ID:
        try:
            if msg_type == "Video":
                await acc.send_message(CUSTOM_CHANNEL_ID, msg.video, entities=msg.entities)
            else:
                await acc.copy_message(CUSTOM_CHANNEL_ID, msg.chat.id, msg.id)
        except Exception as e:
            print(f"Failed to forward message to custom channel: {e}")
    ...
    if CUSTOM_CHANNEL_ID:
        try:
            if msg_type == "Animation":
                await acc.send_message(CUSTOM_CHANNEL_ID, msg.animation, entities=msg.entities)
            else:
                await acc.copy_message(CUSTOM_CHANNEL_ID, msg.chat.id, msg.id)
        except Exception as e:
            print(f"Failed to forward message to custom channel: {e}")
    ...
    if CUSTOM_CHANNEL_ID:
        try:
            if msg_type == "Sticker":
                await acc.send_message(CUSTOM_CHANNEL_ID, msg.sticker, entities=msg.entities)
            else:
                await acc.copy_message(CUSTOM_CHANNEL_ID, msg.chat.id, msg.id)
        except Exception as e:
            print(f"Failed to forward message to custom channel: {e}")
    ...
    if CUSTOM_CHANNEL_ID:
        try:
            if msg_type == "Voice":
                await acc.send_message(CUSTOM_CHANNEL_ID, msg.voice, entities=msg.entities)
            else:
                await acc.copy_message(CUSTOM_CHANNEL_ID, msg.chat.id, msg.id)
        except Exception as e:
            print(f"Failed to forward message to custom channel: {e}")
    ...
    if CUSTOM_CHANNEL_ID:
        try:
            if msg_type == "Audio":
                await acc.send_message(CUSTOM_CHANNEL_ID, msg.audio, entities=msg.entities)
            else:
                await acc.copy_message(CUSTOM_CHANNEL_ID, msg.chat.id, msg.id)
        except Exception as e:
            print(f"Failed to forward message to custom channel: {e}")           
    ...
    if CUSTOM_CHANNEL_ID:
        try:
            if msg_type == "Photo":
                await acc.send_message(CUSTOM_CHANNEL_ID, msg.photo, entities=msg.entities)
            else:
                await acc.copy_message(CUSTOM_CHANNEL_ID, msg.chat.id, msg.id)
        except Exception as e:
            print(f"Failed to forward message to custom channel: {e}")           
