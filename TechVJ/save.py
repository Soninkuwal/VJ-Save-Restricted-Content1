...
# Custom Replace Words command
@Client.on_message(filters.command(["replace"]))
async def replace_words_handler(client: Client, message: Message):
    user_id = message.from_user.id
    
    try:
        # Ask the user for the word to replace and the replacement word, separated by a comma
        await message.reply_text("Please send the word to replace and the new word separated by a comma (e.g., oldword,newword).")
        replace_message = await client.listen.Message(filters.text & filters.user(user_id), timeout=60)
        old_word, new_word = replace_message.text.split(",")
        old_word = old_word.strip()
        new_word = new_word.strip()

        # Update the database with the custom replace words
        database.update_one({'chat_id': user_id}, {'$set': {'replace_words': {old_word: new_word}}}, upsert=True)
        await message.reply_text(f"Successfully set to replace '{old_word}' with '{new_word}'.")

    except asyncio.TimeoutError:
        await message.reply_text("Timeout. Please try /replace again.")
    except ValueError:
        await message.reply_text("Invalid format. Please use 'oldword,newword'.")
    except Exception as e:
        await message.reply_text(f"An error occurred: {e}")

# Custom Delete Words command
@Client.on_message(filters.command(["delete_words"]))
async def delete_words_handler(client: Client, message: Message):
    user_id = message.from_user.id
    
    try:
        # Ask the user for the words to delete, separated by commas
        await message.reply_text("Please send the words to delete, separated by commas (e.g., word1,word2,word3).")
        delete_message = await client.listen.Message(filters.text & filters.user(user_id), timeout=60)
        delete_words_list = [word.strip() for word in delete_message.text.split(",")]
        
        # Update the database with the custom delete words
        database.update_one({'chat_id': user_id}, {'$set': {'delete_words': delete_words_list}}, upsert=True)
        await message.reply_text(f"Successfully set to delete the following words: {', '.join(delete_words_list)}.")

    except asyncio.TimeoutError:
        await message.reply_text("Timeout. Please try /delete_words again.")
    except Exception as e:
        await message.reply_text(f"An error occurred: {e}")
...
async def handle_private(client: Client, acc: Client, message: Message, chatid: int or str, msgid: int, forward_to_channel: bool = False):
    try:
        ...
        # Custom Replace and Delete Words
        user_data = database.find_one({'chat_id': message.from_user.id})
        replace_words = get(user_data, 'replace_words', {})
        delete_words = get(user_data, 'delete_words', [])
        ...
        if "Text" == msg_type:
            try:
                ...
                # Apply custom replace and delete words logic to text messages as well
                for word in delete_words:
                ...
                if forward_to_channel:
                    await acc.send_message(chat, text_content, entities=msg.entities)
                else:
