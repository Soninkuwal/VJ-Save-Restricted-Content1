    ...
    if "https://t.me/" in message.text:
        ...
        for msgid in range(fromID, toID+1):
            ...
            # bot
                ...
                except Exception as e:                    if ERROR_MESSAGE == True:
                        await client.send_message(message.chat.id, f"Error: {e}", reply_to_message_id=message.id)
    ...
    try:
        ...
        if "Text" == msg_type:
            ...
            # forward custom channel ID
                # await client.forward_messages(FORWARD_CHANNEL_ID, message.chat.id, sent_msg.id)  #Fix: No sent_msg
            except Exception as e:
        ...
        try:
            ...
            if not file:
                logging.warning(f"Download media returned None")
                await client.edit_message_text(chat, smsg.id, "**Download Failed: No file returned.**")  # Notify User
                return
            ...
            if os.path.getsize(file) == 0:
                ...
                try:
                    ...
                    logging.error(f"Error deleting empty file: {e}")
            ...
            try:
                ...
                logging.error(f"Error removing downstatus.txt: {e}")

        except FloodWait as e:
            logging.warning(f"FloodWait during download: {e}")
            await asyncio.sleep(e.value)
            return  # Or retry the download after waiting
        except Exception as e:
        ...
        if "Document" == msg_type:
            ...
            # forward custom channel ID
                ...
                logging.warning(f"FloodWait during upload: {e}")
                await asyncio.sleep(e.value)
                return
            except Exception as e:
                logging.error(f"Error forwarding message: {e}")
