            ...
            if txt:
                ...
                if percent_match:
                    ...
                    if abs(current_percent - last_updated_percent) >= 5:
                         await client.edit_message_text(chat, message.id, f"**{text_prefix} : <a href=https://t.me/SONICKUWALUPDATEKANHA>JOIN UPDATED CHANNEL</a>** **{txt}**")
...
# Forwarding function
async def forward_to_channel(client, chat_id, message_id):
    try:
      user_channel_id = await db.get_channel(chat_id)
      if user_channel_id:
           await client.forward_messages(user_channel_id, chat_id, message_id)
      else:
...
# Set custom channel command
@Client.on_message(filters.command(["setchannel"]))
async def set_custom_channel(client: Client, message: Message):
    try:
        channel_id = message.text.split(" ")[1]
        if channel_id.startswith("-100"):
            await db.add_channel(message.from_user.id, int(channel_id))
            await message.reply_text(f"**Custom channel has been set to {channel_id}**")
        else:
            await message.reply_text("**Please give a valid channel id**")
    except IndexError:
        await message.reply_text("**Please provide a channel ID to set. Use like: `/setchannel -100xxxxxxxx`**")
    except Exception as e:
         logging.error(f"Error setting custom channel: {e}")
         await message.reply_text(f"**Error setting the channel: {e}**")
...
async def handle_private(client: Client, acc, message: Message, chatid: int, msgid: int):
    ...
    if msg_type == "Text":
        if msg.text:
            try:
                ...
                await forward_to_channel(client, message.chat.id, sent_msg.id)
            except Exception as e:
                 logging.error(f"Error sending text message: {e}")
                 if ERROR_MESSAGE:
                     await client.send_message(message.chat.id, f"Error Sending Text: {e}", reply_to_message_id=message.id, parse_mode=enums.ParseMode.HTML)
            return
        return

    smsg = await client.send_message(message.chat.id, '**Downloading**', reply_to_message_id=message.id)
    download_task = asyncio.create_task(update_status(client, f'{message.id}downstatus.txt', smsg, chat, "Downloaded"))
    file = None
    try:
    ...
    upload_task = asyncio.create_task(update_status(client, f'{message.id}upstatus.txt', smsg, chat, "Uploaded"))
    try:
        if msg.caption:
            caption = msg.caption
        else:
        ...
        if msg_type == "Document":
             if msg.document.file_size == 0:
                await client.send_message(chat, "**Skipping 0kb Document**", reply_to_message_id=message.id)
                return
             ph_path = None
             ...
             if msg.video.file_size == 0:
                await client.send_message(chat, "**Skipping 0kb Video**", reply_to_message_id=message.id)
                return
             ph_path = None
             if msg.video.thumbs:
                try:
         ...
         if ERROR_MESSAGE:
              await client.send_message(message.chat.id, f"Error: while sending: {e}", reply_to_message_id=message.id)
